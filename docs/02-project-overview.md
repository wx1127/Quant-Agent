...
截至 0.7.0，M4 的 Agent 核心已完成：决策快照、受控工具、显式状态机、证据化
回答、安全防护、十二个黄金场景和审计回放均已有实现与测试。Web 对话入口、身份认证
和人工审批界面归 P7；它们不会向 Agent 注册人工审批或 Kill Switch 恢复权限。

截至 0.8.0，P7 已完成认证 API、研究与组合接口、人工审批、模拟提交、日报及 Web
工作台。审批能力位于独立 API 并要求 Approver 角色，Agent 对话入口仍不拥有审批、
Kill Switch 恢复或真实订单提交权限。
