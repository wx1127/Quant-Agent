...
P6-T07 的 `AgentSecurityBoundary` 为每次编排生成服务端信封：运行 ID、决策 ID、固定模式、目标、
状态和可用工具均来自已验证快照/中央权限矩阵，不读取用户文本中的模式声明。用户要求切换模式、
批准订单、修改风控、停用 Kill Switch、绕过规则或索取密钥时，输入被拒绝，只保留不含原文的安全发现。
普通用户文本中的密钥形态会被替换为 `[REDACTED]`。

外部新闻、公告和网页先通过 `ContentSanitizer`；通过初筛的内容仍包装为
`UntrustedContentBlock(instruction_authority=False)`，被隔离为数据而不是系统消息。被隔离或拒绝的内容
不携带原文。只要一次编排带有外部内容，`ARTIFACT_WRITE`、模拟提交和实盘提交工具都会从模型可见
集合中移除，外部文本不能借机创建草案或执行订单；后续动作必须由新的、无外部指令污染的控制流程发起。

最终答案发布前递归扫描所有模型生成字符串，阻断 API Key、Bearer/JWT、供应商 Token、秘密引用、
长账户数字和当前决策账户标识。阻断错误只返回固定安全错误，不包含敏感片段。工具白名单仍由 P6-T02
和 P6-T05 的注册器/状态机执行，P6-T07 不复制或替代执行权限。
