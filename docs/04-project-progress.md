...
- **任务定义**：
  ...
  - `[已完成] P8-T08-E03-06` 退出信号记录主规则、全部命中规则、优先级、观测值、阈值、数量和状态；T+1 冻结时标记 `BLOCKED_T_PLUS_ONE`，不得生成不可执行卖单。
  - `[已完成] P8-T08-E03-07` 补齐前置过滤、真实行业主线、全部退出类型、规则优先级和 T+1 阻塞测试，并通过全量质量门禁。
