...
## P8-T08-E05：8 月 5 日新规则 PAPER 候选与订单重算

- **状态**：已完成
- **执行结果**：Tushare 限频恢复后，使用 25 个冻结交易日行情和 5,537 只股票行业快照生成 append-only `PAPER_PREVIEW`；未覆盖原日记录、账户、草稿、报告或 8 月 6 日人工退出覆盖单。
- **任务定义**：
  - `[已完成] P8-T08-E05-01` 冻结完整 Tushare 股票名称与行业快照，拒绝使用仅覆盖 490 只股票的局部研究快照冒充全市场数据。
  - `[已完成] P8-T08-E05-02` 按候选前置过滤 v2 和退出规则 v1 重算行业主线、候选、排除项、退出信号及次日模拟草稿。
  - `[已完成] P8-T08-E05-03` 生成 `recalculations/2026-08-05-rules-v2.json` 和对应行业快照；模式固定为 `PAPER_PREVIEW`，实盘连接为零。
  - `[已完成] P8-T08-E05-04` 验证原空草稿及 8 月 6 日人工退出覆盖单 SHA-256 均未变化；实际待执行批次仍为人工覆盖单的 5 笔创业板卖出，不执行预览中的买单。
  - `[已完成] P8-T08-E05-05` 删除一次性重算脚本，运行全量测试并提交 feature 分支。
- **重算摘要**：合规候选 10 只，过滤项 204 只；预览草稿含 5 笔卖出及 1 笔泛微网络买入，但预览不进入调度。8 月 6 日真实 PAPER 待执行覆盖单仍只有 5 笔卖出、合计 52,900 股。

## P8-T08-E04：8 月 6 日早盘创业板 PAPER 全部退出

- **状态**：运行中（草稿已生成，待 2026-08-06 早盘执行）
- **用户指令**：2026-08-05 收盘后要求下一交易日早盘卖出模拟账户全部创业板持仓。
- **任务定义**：
  - `[已完成] P8-T08-E04-01` 核对账户中 5 只创业板持仓、总数量、T+1 冻结状态和原 8 月 5 日空草稿。
  - `[已完成] P8-T08-E04-02` 新增 append-only `pending_overrides/2026-08-06.json`，包含 5 笔全量卖出草稿和 `USER_DIRECTED_CHINEXT_EXIT` 原因，不覆盖原草稿。
  - `[已完成] P8-T08-E04-03` 日终入口只接受显式 `mode=PAPER`、`manual_override=true` 且完整列出被替代信号日的覆盖单；验证原草稿 SHA-256 未变化。
  - `[待执行] P8-T08-E04-04` 2026-08-06 早盘由 `PaperBroker` 先释放 T+1 数量，再按当日开盘价、5 bps 滑点、佣金和卖出税模拟成交。
  - `[待验证] P8-T08-E04-05` 收盘后核对 5 笔成交、现金、费用滑点、剩余创业板仓位为零及日报证据。
- **不冲突约束**：覆盖单只替代 8 月 6 日的 PAPER 待执行批次；不得连接券商、不得 LIVE、不得同日回看成交、不得修改 `pending/2026-08-05.json`。

## P8-T08-E03：候选前置过滤与分层退出规则

- **状态**：已完成
- **目标**：禁止无效股票进入候选评分和排名，并为 PAPER 持仓建立独立于候选轮动的强制退出层。
- **范围/归属**：历史时点候选入口、P8 日终决策、持仓状态、次日卖出草稿及审计证据。
- **任务定义**：
  - `[已完成] P8-T08-E03-01` 在计算候选分数前排除港股、涨跌幅限制高于 10% 的板块，以及当日实际涨跌幅绝对值超过 10% 的股票；不得先进入 Top 10 再过滤。
  - `[已完成] P8-T08-E03-02` 将过滤结果写入 `market.candidate_exclusions` 和 `next_day_order_draft.excluded_buy_candidates`，过滤标的不得拥有候选分数或排名。
  - `[已完成] P8-T08-E03-03` 记录入场日期、交易日持仓天数、历史最高收盘价和最后评估日期，状态随次日草稿传递；旧账户首次接入时明确标记推断状态。
  - `[已完成] P8-T08-E03-04` 将 Tushare 行业随股票快照冻结并接入 P8 主线计算，禁止以沪深/创业板等交易所板块充当主线；主线退出按行业主线判断。
  - `[已完成] P8-T08-E03-05` 实现单股止损、固定止盈、移动止盈、跌破 MA5/MA10、行业主线退出、大幅低开、放量下跌、最大持仓交易日及候选轮动退出。
  - `[已完成] P8-T08-E03-06` 退出信号记录主规则、全部命中规则、优先级、观测值、阈值、数量和状态；当日买入仓位只生成下一交易日草稿，状态标记 `READY_AFTER_T_PLUS_ONE_RELEASE`，由次日执行器解冻后卖出，严禁同日成交。
  - `[已完成] P8-T08-E03-07` 补齐前置过滤、真实行业主线、全部退出类型、规则优先级和 T+1 次日解冻测试，并通过全量质量门禁。
- **不冲突约束**：E03 只改变新生成的候选和次日草稿，不覆盖 2026-08-05 及之前的 append-only 证据；所有退出仍在收盘后产生信号并于下一交易日由 `PaperBroker` 模拟执行。
- **验收口径**：候选输出只含合规股票；强制风控优先于普通轮动；同一持仓可保留多个退出原因；无同日收盘回看成交、无 LIVE 连接。
- **验收证据**：`pytest -q` 176 passed、总覆盖率 90.98%；`ruff check packages tests` 与 `mypy packages apps` 通过。
...
## P8-T08-E02：涨跌幅限制超过 10% 与港股买入排除规则

- **状态**：已完成
- **日期**：2026-08-05
- **范围/归属**：P8 模拟盘次日买入草稿生成与日报排除证据。
- **交付物**：
  - 买入草稿硬过滤涨跌幅限制超过 10% 的板块：创业板 `CN.SZ.300*`、`CN.SZ.301*`，科创板 `CN.SH.688*`、`CN.SH.689*`，北交所 `CN.BJ.*`。
  - 买入草稿硬过滤交易所段为 `HK` 的港股。
