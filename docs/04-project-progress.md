...
> 文档状态：执行版 v0.1  
> 初始日期：2026-07-30  
> 当前阶段：P8 本地系统验证基础已完成；部署链等待 P0 远程持续集成确认
> 关联文档：  
...
| P8 | 系统验证、部署与实盘准备 | 10 | 4 | 验证基础已完成，部署链待前置 |
| **合计** |  | **75** | **68** | **P8 进行中** |
...
| 2026-07-30 | P8-T02 | 未开始 | 已完成 | 建立十二个系统黄金场景、固定预期、自动断言和 JSON 回放报告 | 全部场景通过；坏数据、审批价格偏离、幂等重试和风控绕过被覆盖 | Codex |
...
- 等待 `feature/p8-system-validation` 分支审查与前序堆叠分支合并，不直接推送 `main`；
- 在 GitHub 页面确认远程 CI 后，将 **P0-T07** 标记为 `已完成`；
- P0-T07 完成后开始 **P8-T05：建立部署与回滚流程**；
- P8-T06～P8-T10 继续按部署、监控、20 个交易日影子运行、3 个月模拟盘和合规前置顺序执行，不提前改变状态。
