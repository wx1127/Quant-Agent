...
- **任务定义**：
  ...
  - `[已完成] P8-T08-E03-04` 实现单股止损、固定止盈、移动止盈、跌破 MA5/MA10、主线退出、大幅低开、放量下跌、最大持仓交易日及候选轮动退出。
  - `[已完成] P8-T08-E03-05` 退出信号记录主规则、全部命中规则、优先级、观测值、阈值、数量和状态；T+1 冻结时标记 `BLOCKED_T_PLUS_ONE`，不得生成不可执行卖单。
  - `[已完成] P8-T08-E03-06` 补齐前置过滤、全部退出类型、规则优先级和 T+1 阻塞测试，并通过全量质量门禁。
