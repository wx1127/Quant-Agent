...
- 用户明确要求调整下一交易日 PAPER 订单时，原 `pending/` 草稿仍保持 append-only；新批次写入 `pending_overrides/<execute_on>.json`。
- 覆盖单必须包含 `mode=PAPER`、`manual_override=true`、执行日、人工授权说明、被替代信号日列表、原草稿路径及哈希。
- 执行入口必须验证覆盖单完整列出该执行日全部原信号日；缺失、日期不一致或非 PAPER 模式立即失败留痕。
- 覆盖单仍由 `PaperBroker` 执行，服从下一交易日、T+1、费用、税费、滑点、涨跌停和账实核对规则；人工指令不授权真实交易。
...
- 主线必须使用当日冻结的 Tushare 股票行业字段计算；沪市主板、深市主板、创业板、科创板、北交所等交易所板块不得作为主线。行业快照缺失时当日任务失败留痕，不降级为交易所板块。
- 退出层按收盘数据检查：成本回撤 8% 止损、盈利 20% 固定止盈、峰值盈利达到 10% 后从峰值回撤 8% 移动止盈、向下跌破 MA5/MA10、退出当前行业主线、低开 5%、跌幅 7% 且成交量达到前五日均量 1.5 倍、持仓满 20 个交易日，以及候选轮动退出。
- 强制风控优先于候选轮动；同一股票允许同时命中多条规则，证据必须保留全部原因及主规则。
- 所有退出信号在收盘后生成，只能在下一交易日模拟卖出。当日买入的 T+1 冻结仓位标记为 `READY_AFTER_T_PLUS_ONE_RELEASE`，可以生成次日卖出草稿，但必须由次日执行器先解冻后成交，严禁同日卖出或伪造成交。
