...
- 当前阻塞：运行环境未配置 `MARKET_DATA_TOKEN`，不得用 Fake Provider 或历史回放
  冒充实时影子运行。

计划窗口依据上交所 2026 年休市安排和周一至周五交易规则生成。正式开始前必须通过
Tushare `trade_cal` 接口重新冻结交易日历并保留哈希；若提供商日历与计划不一致，以
提供商冻结结果为准并更新会话，不覆盖既有证据。
