...
调用链。公告在线链路仍待真实 Tushare Token 验收，远程 CI 仍待确认；生产领域数据 source、
耐久草案仓储、显式状态机、输出证据、轨迹评测、API 和 Web 尚未完成。当前台账为 58/84 项
任务完成，不能把当前版本视为可自动实盘系统。详细状态见：
...
最近一次本地完整门禁（2026-09-09）为 1595 项测试通过、总覆盖率 90.54%，同时通过 Ruff
格式/代码检查和 Mypy（165 个源文件）类型检查。该数字是当前工作树快照，不替代 CI 与真实
数据源验收。
...
P6-T04 交付，Agent 状态机、完整运行轨迹和制品聚合回放仍属于后续任务。可信输入、存储及当前
支持范围详见
